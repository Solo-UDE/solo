...
| File | Purpose |
|---|---|
| `solo/.envrc` | direnv loader — sources `infra/.env.dev` into the shell env when you `cd` into `solo/` |
| `solo/infra/.env.dev` | Public CDK outputs (Cognito pool IDs, RDS endpoint, API URLs). **Regenerated by `bun run deploy:dev`.** Committed because all values are public identifiers. |
| `solo/.env` | Node/Tauri runtime env. Currently has `ANTHROPIC_KEY` and `CLAUDE_CODE_OAUTH_TOKEN` only. |

Required env vars the desktop reads at runtime (from `auth_commands.rs`):
...
- `SOLO_COGNITO_USER_POOL_ID=us-east-1_S9C2mSy8E`

All four are in `infra/.env.dev`, so direnv handles it automatically. If you need to verify: `env | grep SOLO_COGNITO` should show four values before running `bun run dev`.
...
# direnv loads infra/.env.dev automatically. First-time setup:
direnv allow

# Sanity check: SOLO_COGNITO_* should all be set
env | grep SOLO_COGNITO

# Launch with the auth guard actually mounted
VITE_AUTH_ENABLED=1 bun run dev
```

### Why `VITE_AUTH_ENABLED=1`
